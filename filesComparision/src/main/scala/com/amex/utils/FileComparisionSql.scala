           ...
           */
            ...
            if ( matchCount.count() > 0) {
              ...
              val TableName:String = "FileComparision.filecomaprision"+currentTimestamp
               ...
               createStmnt1 = createStmnt.append("create table if not exists ").append(TableName).append(" as select ")

              createStmnt = createStmnt.append("select ")
              ...
              /*ColumnList.map(x => {
                if (CoulumnPercent.contains(x) && SchemaDT.get(x).getOrElse("") != "StringType") {
                  val bucketPercentage = CoulumnPercent.get(x).getOrElse("")
                  for (buc <- bucketPercentage.split(",", -1)) {
                    ...
                    if (buc.length == 1)
                      ...
                      perc = "1." + buc
                    ConditionBucketList.+=("sum(case when nvl(a." + x + ",0)*" + perc + " > nvl(b." + x + ",0) then 1 when nvl(a." + x + ",0) < nvl(b." + x + ",0) then 1 else 0 end) as " + x + "_" + buc + "Percent_Stat,")
              ...
              ColumnList.map(x => {
                if (CoulumnPercent.contains(x) && SchemaDT.get(x).getOrElse("") != "StringType") {
                  ...
                  for (buc <- bucketPercentage.split(",", -1)) {
                    ...
                    val BucketConditionString: String = "sum( case when (ABS(((nvl(b."+x+",0) - nvl(a."+x+",0))/nvl(a."+x+",0))*100) < "+buc+".1) or (length(b."+x+") = 0 AND length(a."+x+") = 0) then 1 else 0 end ) as " + x + "_" + buc + "_Stat,"
                    ConditionBucketList.+=(BucketConditionString)
                    //ConditionBucketList.+=("sum(case when nvl(a." + x + ",0)*" + perc + " > nvl(b." + x + ",0) then 1 when nvl(a." + x + ",0) < nvl(b." + x + ",0) then 1 else 0 end) as " + x + "_" + buc + "_Stat,")
                    ColNames.+=(x + "_" + buc + "_Stat")
                    BucketList.+=(x)
                  }
                }
              ...
              val statsDf = spark.sql(s"$stmnt").persist()
              ...
              if(BucketList.size > 0) {
                percnetageBuclket.foreach(x => {
                  columnStatHeader.append(x.toString.concat("% Match Count,").concat(x.toString).concat("% Match Percentage,").concat(x.toString).concat("% UnMatched Count,").concat(x.toString).concat("% UnMatched Percentage,"))
                })
              }
              ...
              ColumnList.foreach(x => {
                println("Column "+ x)
                val clmn = x
                val Columnvalue = statsDf.select(x+"_Stat").collect.map(_(0))
                ...
                if (CoulumnPercent.contains(x) && SchemaDT.get(x).getOrElse("") != "StringType") {
                  ...
                  for (buc <- bucketPercentage.split(",", -1)) {
                    ...
                    println(columnName)
                    val BucketColumnvalue = statsDf.select(columnName).collect.map(_(0))
                    ...
                    statMsg.append( "," + bucketMatchCnt + "," + bucketMatchPercentage + "%," + bucketUnmatchedCnt + "," + bucketUnmatchedPercentage +"%")
                  }
                  statMsg.append(",")
                }
                else
