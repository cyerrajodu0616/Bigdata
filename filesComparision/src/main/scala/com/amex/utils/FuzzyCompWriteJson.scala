...
object FuzzyCompWriteJson {
  def main(args: Array[String]): Unit = {
    ...
    val ConfigFileName = propertyFilePercentage.split("/").last
 ...
 */
      ...
      for (x <- importanceList) {
        coulumnImportance.+=((x \ "name").extract[String] -> (x \ "value").extract[String])
      }
      ...
      for (x <- columnType) {
        val Percent = (x \ "cols").children
        for (y <- Percent)
          colTypeHashMap += (y.extract[String] -> (x \ "type").extract[String])
      }
