                        ...
                        if (RequiredColumnList.size == 0) {
                          ColumnList.map(x => {
                            if (CoulumnPercent.contains(x) && SchemaDT.get(x).getOrElse("") != "StringType") {
                              if (CoulumnPercent.contains(x)) {
                                ...
                                for (buc <- bucketPercentage.split(",", -1)) {
                                  ...
                                  val BucketConditionString: String = " case " +
                                    ...
                                    s"WHEN ABS(round(b.$x,6)) >= 1 and ABS(round(b.$x,6)) <= 10 and round(b.$x,6) is NOT NULL and round(a.$x,6) is NOT NULL THEN 0 else null end  as ${x}_${percentage}_Stat,"

                                  val BucketCountConditionString: String = " CASE " +
                                    ...
                                    s"else null end  as ${x}_${percentage}_cnt,"


                                  ConditionBucketList.+=(s"case when (ABS(round(b.$x,6)) < 1  or ABS(round(round(b.$x,6),6)) >  10)  then ABS((round(b.$x,6) - round(a.$x,6))/round(b.$x,6)) else NULL end as ${x}_${percentage}_Stat_value,")
                                  ConditionBucketList.+=(BucketConditionString)
                                  ConditionBucketList.+=(BucketCountConditionString)
                                  ColNames.+=(x + "_" + percentage + "_Stat")
                                  BucketList.+=(x)
                                }
                              }
                            }
                            ...
                            if (SchemaDT.get(x).getOrElse("") != "StringType") {
                              ConditionBucketList.+=(s"case when round(a.$x,6)*1 = round(b.$x,6)*1 then 1 when a.$x is not null and b.$x is not null and  a.$x != b.$x then 0 else NULL end as ${x}_Stat,")
                              ConditionBucketList.+=(s"round(a.$x,6)  as ${x}_f1_Only,")
                              ConditionBucketList.+=(s"round(b.$x,6)  as ${x}_f2_Only,")

                            } else {
                              ConditionBucketList.+=(s"case when a.$x = b.$x then 1 when a.$x is not null and b.$x is not null and  a.$x != b.$x then 0 else NULL end as ${x}_Stat,")
                              ConditionBucketList.+=(s"a.$x  as ${x}_f1_Only,")
                              ConditionBucketList.+=(s"b.$x  as ${x}_f2_Only,")
                            }

                            ConditionBucketList.+=(s" case when a.$x = b.$x then 1 " +
